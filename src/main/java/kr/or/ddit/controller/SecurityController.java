...
public class SecurityController {
	 ...
	 *		        대상        |     화면          |   접근정책
	 *     ────────────────────────────────────────────────────
	 *      일반게시판	   |   목록화면        | 모두가 접근 가능합니다.
	 *      	       |   등록화면        | 로그인한 회원만 접근 가능합니다.
	 *     ────────────────────────────────────────────────────
	 *      공지게시판        |   목록화면        | 모두가 접근 가능합니다.
	 ...
	 * 				
	 * 				** 우리는 현재 kr.or.ddit를 기준으로 base 패키지로 설정되어 있기 때문에 위와 같은 설정을 하지는 않는다.
	 ...
	 *		- 공지사항 게시판 등록 페이지 
	 *			> 접근 제한에 걸려 스프링 시큐리티가 제공하는 로그인 페이지가 나타나고,
	 *			회원 권한을 가진 계정으로 접근 시, 공지사항 게시판 등록 페이지는 관리자 권한만 접근 가능하므로da
	 ...
	 *  				: 빈 등록에 사용되는 클래스는 security 패키지 아래에 있는 CustomAccessDeniedHanler클래스
	 *  
	 *  	접근 거부 처리자 클래스 정의
	 *  	- 사용자가 정의한 접근 거부 처리자
	 *  		> CustomAccessDeniedHandler 클래스 정의
	 *  			- AccessDeniedHandler 인터페이스를 참조받아서 handel 메소드를 재정의 하여 사용합니다.
	 * 				: 우리는 접근이 거부되었을 때 빈으로 등록해둔 CustomAccessDeniedHandler 클래스가 발동해
	 * 				해당 메소드가 실행되고 response 내장객체를 활용하여 /accessError URI로 이동하여 접근 거부시
	 * 				보여질 페이지로 이동하지만 이곳에서 더 많은 로직을 처리길 원하면 request, response 내장객체를 이용하여
	 * 				다양한 처리가 가능하다. 
	 * 
	 * 		화면 설정(회원 권한으로 접근을 가정한다.)
	 * 		- 일반 게시판 등록 페이지
	 ...
	 *  	이런 경우에 AuthenticationSuccessHandle이라는 인터페이스를 구현해서 로그인 성공 처리자로 지정할 수 있습니다.
	 *  
	 *  	환경 설정
	 *  	- 스프링 시큐리티 설정
	 *  		> security-context.xml 설정
	 *  			- customLoginSuccess 빈 등록
	 *  			- sedurity:form-login login-page="/login" authentication-success-handler-ref="customLoginSuccess"/> 추가
	 *  
	 *  		로그인 성공 처리자 클래스 정의
	 *  			- 로그인 성공 처리자
	 ...
	 *  
	 *  		** logout 경로는 스프링에서 제공하는 /logout 경로를 설정한다.
	 ...
	 *    	  제대로 할려면 생성된 사용자 테이블(users)에 비밀번호를 암호화하여 저장해야 합니다.
	 *    	  테스트를 위해서 생성한 데이터는 암호화를 처리하지 않고 로그인 하면 당연히 로그인 에러가 발생할 것입니다.(암호화된 비밀번호가 날라가는게 아니라서)
	 ...
	 *  
	 *  	> 비밀번호를 입력하면 암호화된 비밀번호로 인코딩 되는데, 암호화된 비밀번호와 디비 테이블에 있는 암호화된 비밀번호가 일치한지를 파악 한 후,
	 ...
	 * 		> 기존 memberMapper에서 사용중인 read를 기반으로 한 readById를 새롭게 재정의 하여 사용하겠음
	 ...
	 * 
	 * 14. 자동 로그인
	 * - 로그인 하면 특정 시간 동안 다시 로그인 할 필요가 없는 기능이다.
	 * 스프링 시큐리티는 메모리나 데이터베이스를 사용하여 처리한다.
	 * 기능을 구현하기 위해 <security:remember-me> 태그를 이ㅛㅇㅇ하여 시큐리티 설정 파일을 수정한다.
	 * 
	 * 		데이터베이스 테이블
	 * 		- persistent_longins 테이블 준비
	 * 
	 * 		환경설정
	 * 		- 스프링 시큐리티 설정
	 * 		> security-context.xml 실정
	 * 		> <security:remember-me date-source-ref="dataSource" token-validity=seconds="604800"/> 태그 설정
	 ...
	 * 			> 자동 로그인이 remember-me에 의해 실해오딤을 확인할 수 있습니다.
	 * 		
	 * 15. 스프링 시큐리티 어노테이션
